 ...
 * - Smart retry: 429 → reports to manager + rotates key; 503/500 → exponential
 * backoff;
...
public class GeminiService {
    ...
    public String askAgent(String systemPrompt, String userTask) {
        ...
        // Read model from system property (set via Settings dialog), default to 1.5
        // flash
        ...
        for (int attempt = 0; attempt < MAX_RETRIES; attempt++) {
            ...
            try {
                ...
                if (lastError.contains("503") || lastError.contains("500")
                    ...
                    try {
                        Thread.sleep((long) Math.pow(2, Math.min(attempt + 1, 6)) * 1000);
    ...
    public static void main(String[] args) {
        ...
        if (key != null)
            APIKeyManager.getInstance().setKeys(key);
